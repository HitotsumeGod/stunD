...
struct errep *poll_stun_servers(int af, int amou)
{
        ...
        if (amou < 0) {
                ERREP(err, fnname, "amount argument was less than zero");
                return err;
        } else if (amou == 0 || amou > num_sservers)
                amou = num_sservers;
        memset(&sai, 0, sizeof(sai));
        if ((cargo = malloc(sizeof(struct sockaddr_storage *) * (cargolen = 2))) == NULL) {
                ERREP(err, fnname, "cargo could not be allocated memory");
                return err;
        }
        sai.ai_family = af;
        ...
        if ((sock = socket(spai -> ai_family, spai -> ai_socktype, spai -> ai_protocol)) == -1) {
                ERREP(err, fnname, "socket could not be created");
                return err;
        }
        if (bind(sock, spai -> ai_addr, spai -> ai_addrlen) == -1) {
                ERREP(err, fnname, "socket could not be bound to local address");
                return err;
        }
        freeaddrinfo(spai);
        for (; count < amou; count++) {
                if (count == cargolen) {
                        if ((dummy = realloc(cargo, sizeof(struct sockaddr_storage *) * (cargolen *= 2))) == NULL) {
                                ERREP(err, fnname, "resizing with realloc() could not be completed");
                                return err;
                        }
                        cargo = dummy;
                }
...
struct errep *poll_stun_servers_by_name(int af, struct stun_server **servs)
{
        ...
        if (!servs) {
                ERREP(err, fnname, "stun_server array passed as NULL");
                return err;
        }
        memset(&sai, 0, sizeof(sai));
        if ((cargo = malloc(sizeof(struct sockaddr_storage *) * (cargolen = 2))) == NULL) {
...
struct errep *poll_recv_test(char *host)
{
        ...
        if (setsockopt(sock, SOL_SOCKET, IP_MTU_DISCOVER, &val, sizeof(val)) == -1) {
                ERREP(err, fnname, "socket nonblocking option could not be set");
                return err;
        }
        if (bind(sock, spai -> ai_addr, spai -> ai_addrlen) == -1) {
                ERREP(err, fnname, "socket could not be bound to local address");
        ...
        for (int i = 0; i < 100; i++)
                if (sendto(sock, pmsg, strlen(pmsg), 0, spai -> ai_addr, spai -> ai_addrlen) == -1) {
                        ERREP(err, fnname, "failed to send a holepuncher datagram");
                        return err;
                }
        freeaddrinfo(spai);
        if ((siz = recvfrom(sock, recvbuf, sizeof(recvbuf), 0, NULL, NULL)) == -1) {
                ERREP(err, fnname, "failed to recover from exterior host");
                return err;
        }
        if ((dump = fopen("packet.cap", "wb")) == NULL) {
                ERREP(err, fnname, "failed to open packet dump file");
                return err;
        }
