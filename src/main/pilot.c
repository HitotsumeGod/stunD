...
int main(int argc, char *argv[])
{
        ...
        if (argc == 1) {
                if ((res = poll_stun_servers(AF_INET, num_sservers)) -> msg != NULL) {
                        fprintf(stderr, "%s", ptools_format_errors(res));
                        return EXIT_FAILURE;
                }
                ...
                if (argc == 2) {
                        ...
                        servs[argc - 2] = NULL;
                        if ((res = poll_stun_servers_by_name(AF_INET, servs)) -> msg != NULL) {
                                fprintf(stderr, "%s", ptools_format_errors(res));
                                return EXIT_FAILURE;
                        }
                        for (int i = 0; i < argc - 2; i++) {
                ...
                if ((res = poll_recv_test(argv[2])) -> msg != NULL) {
                        fprintf(stderr, "%s", ptools_format_errors(res));
                        return EXIT_FAILURE;
                }
                ...
                if ((res = poll_stun_servers(AF_INET, num)) -> msg != NULL) {
                        fprintf(stderr, "%s", ptools_format_errors(res));
                        return EXIT_FAILURE;
                }
        } else {
